...
async function GroupUpdate(Jhon, m, store) {
	if (!m.messageStubType || !m.isGroup) return
	if (global.db?.groups?.[m.chat]?.setinfo && Jhon.public) {
		...
		if (messages[m.messageStubType]) {
			await Jhon.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(m.messageStubParameters[0]?.includes('@') ? [`${m.messageStubParameters[0]}`] : [])]}, { ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else {
			console.log({
...
async function GroupParticipantsUpdate(Jhon, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db?.groups?.[id] && store?.groupMetadata?.[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await Jhon.profilePictureUrl(n, 'image');
				} catch {
...
async function LoadDataBase(Jhon, m) {
	try {
		const botNumber = await Jhon.decodeJid(Jhon.user.id);
...
async function MessagesUpsert(Jhon, message, store, groupCache) {
	try {
		let botNumber = await Jhon.decodeJid(Jhon.user.id);
		...
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) store.groupMetadata ??= await Jhon.groupFetchAllParticipating().catch(e => ({}));
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		const m = await Serialize(Jhon, msg, store, groupCache)
		require('../Jhon')(Jhon, m, msg, store, groupCache);
		if (db?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await Jhon.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await Jhon.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await Jhon.sendFromOwner(global.db?.set?.[botNumber]?.owner?.map(x => x.id) || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		throw e;
...
async function Solving(Jhon, store) {
	Jhon.serializeM = (m) => MessagesUpsert(Jhon, m, store)
	...
	Jhon.getName = (jid, withoutContact  = false) => {
		const id = Jhon.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || (store.groupMetadata[id] ? store.groupMetadata[id] : (store.groupMetadata[id] = Jhon.groupMetadata(id))) || {};
	...
	Jhon.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await Jhon.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Jhon.getName(i + '@s.whatsapp.net')}\nFN:${await Jhon.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Jhon.getName(i + '@s.whatsapp.net')}\nFN:${await Jhon.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/Jhon_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		Jhon.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	Jhon.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await Jhon.query({
			tag: 'iq',
			attrs: {
	...
	Jhon.extractGroupMetadata = (result) => {
	...
	Jhon.groupMetadata = async (jid) => {
		...
		return Jhon.extractGroupMetadata(result);
	};
	
	Jhon.groupFetchAllParticipating = async () => {
		const result = await Jhon.query({ tag: 'iq', attrs: { to: '@g.us', xmlns: 'w:g2', type: 'get' }, content: [{ tag: 'participating', attrs: {}, content: [{ tag: 'participants', attrs: {}}, { tag: 'description', attrs: {}}]}]});
		const data = {};
		const groupsChild = getBinaryNodeChild(result, 'groups');
		if (groupsChild) {
			const groups = getBinaryNodeChildren(groupsChild, 'group');
			for (const groupNode of groups) {
				const meta = Jhon.extractGroupMetadata({
					tag: 'result',
					attrs: {},
					content: [groupNode]
				});
		...
		Jhon.ev.emit('groups.update', Object.values(data));
		return data;
	}

	Jhon.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return Jhon.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	}
	
	Jhon.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return Jhon.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, quotedOptions);
			} else if (mime && mime === 'application/pdf') {
				return Jhon.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('image')) {
				return Jhon.sendMessage(jid, { image: res.data, caption: caption, ...options }, quotedOptions);
			} else if (mime && mime.includes('video')) {
				return Jhon.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, quotedOptions);
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return Jhon.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return Jhon.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, quotedOptions);
			}
		}
	...
	Jhon.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await Jhon.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	Jhon.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await Jhon.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	
	Jhon.sendText = async (jid, text, quoted, options = {}) => Jhon.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
	
	Jhon.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return Jhon.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
	}
	
	Jhon.downloadMediaMessage = async (message) => {
	...
	Jhon.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await Jhon.downloadMediaMessage(message);
	...
	Jhon.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: Jhon.user.id, quoted: m.quoted && m.quoted.fakeObj, ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
		apb.key.id = [...Array(32)].map(() => '0123456789ABCDEF'[Math.floor(Math.random() * 16)]).join('');
		apb.key.fromMe = areJidsSameUser(m.sender, Jhon.user.id);
		if (m.isGroup) apb.participant = m.sender;
		Jhon.ev.emit('messages.upsert', {
			...m,
	...
	Jhon.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await Jhon.getFile(path, true);
		...
		if (isWebpSticker) {
			pathFile = await writeExif(data, {
				...
				author: options.author || db?.set?.[botNumber]?.author || 'Jhondev',
		...
		let anu = await Jhon.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ephemeralExpiration: quoted?.expiration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	Jhon.sendAlbumMessage = async (jid, content = {}, options = {}) => {
		...
		await Jhon.relayMessage(jid, medias.message, { messageId: medias.key.id });
		for (const media of album) {
			const msg = await generateWAMessage(jid, { ...others, ...media }, { upload: Jhon.waUploadToServer });
			...
			await Jhon.relayMessage(jid, msg.message, { messageId: msg.key.id });
		}
		return medias;
	}
	
	Jhon.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await Jhon.relayMessage(msg.key.remoteJid, msg.message, {
	...
	Jhon.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	Jhon.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: Jhon.waUploadToServer });
			const anu = await Jhon.query({
				tag: 'message',
	...
	Jhon.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: Jhon.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await Jhon.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (Jhon.user && Jhon.user.id) {
		const botNumber = Jhon.decodeJid(Jhon.user.id);
		if (global.db?.set[botNumber]) {
			Jhon.public = global.db.set[botNumber].public
		} else Jhon.public = true
	} else Jhon.public = true
...
async function Serialize(Jhon, msg, store, groupCache) {
	const botLid = Jhon.decodeJid(Jhon.user.lid);
	const botNumber = Jhon.decodeJid(Jhon.user.id);
	const m = { ...msg };
	if (!m) return m
	if (m.key) {
		...
		if (!m.isGroup && m.chat.endsWith('@lid')) m.chat = Jhon.findJidByLid(m.chat, store) || m.chat;
		m.sender = Jhon.decodeJid(m.fromMe && Jhon.user.id || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await Jhon.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = groupCache.get(m.chat))
			if (!metadata) {
				metadata = await Jhon.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = Jhon.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === Jhon.decodeJid(Jhon.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, Jhon)
				return await Serialize(Jhon, q, store, groupCache)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(Jhon.decodeJid(m.msg?.contextInfo?.participant), Jhon.decodeJid(Jhon?.user?.id)),
			...
			m.quoted.download = () => Jhon.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				Jhon.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => Jhon.downloadMediaMessage(m)
	
	m.copy = () => Serialize(Jhon, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	
	m.react = (u) => Jhon.sendMessage(m.chat, { react: { text: u, key: m.key }})
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return Jhon.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return Jhon.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return Jhon.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
					}
				} else {
					return Jhon.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
				}
