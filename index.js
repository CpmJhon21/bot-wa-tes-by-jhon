...
async function startJhonBot() {
	const { state, saveCreds } = await useMultiFileAuthState('Jhondev');
	...
	const getMessage = async (key) => {
		...
		return {
			conversation: 'Halo Saya Jhon Bot'
		}
	}
	
	const Jhon = WAConnection({
		logger: level,
		getMessage,
	...
	if (pairingCode && !phoneNumber && !Jhon.authState.creds.registered) {
	...
	await Solving(Jhon, store)
	
	Jhon.ev.on('creds.update', saveCreds)
	
	Jhon.ev.on('connection.update', async (update) => {
		const { qr, connection, lastDisconnect, isNewLogin, receivedPendingNotifications } = update
		if (!Jhon.authState.creds.registered) console.log('Connection: ', connection || false);
		if ((connection === 'connecting' || !!qr) && pairingCode && phoneNumber && !Jhon.authState.creds.registered && !pairingStarted) {
			setTimeout(async () => {
				pairingStarted = true;
				console.log('Requesting Pairing Code...')
				let code = await Jhon.requestPairingCode(phoneNumber);
		...
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.connectionLost) {
				...
				Jhon.end(`Unknown DisconnectReason : ${reason}|${connection}`)
			}
		}
		if (connection == 'open') {
			console.log('Connected to : ' + JSON.stringify(Jhon.user, null, 2));
			let botNumber = await Jhon.decodeJid(Jhon.user.id);
			if (global.db?.set[botNumber] && !global.db?.set[botNumber]?.join) {
				if (my.ch.length > 0 && my.ch.includes('@newsletter')) {
					if (my.ch) await Jhon.newsletterMsg(my.ch, { type: 'follow' }).catch(e => {})
	...
	Jhon.ev.on('contacts.update', (update) => {
		for (let contact of update) {
			let trueJid;
			if (!trueJid) continue;
			if (contact.id.endsWith('@lid')) {
				trueJid = Jhon.findJidByLid(contact.id, store);
			} else {
	...
	Jhon.ev.on('call', async (call) => {
		let botNumber = await Jhon.decodeJid(Jhon.user.id);
		if (global.db?.set[botNumber]?.anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await Jhon.sendMessage(id.from, { text: `Saat Ini, Kami Tidak Dapat Menerima Panggilan ${id.isVideo ? 'Video' : 'Suara'}.\nJika @${id.from.split('@')[0]} Memerlukan Bantuan, Silakan Hubungi Owner :)`, mentions: [id.from]});
					await Jhon.sendContact(id.from, global.owner, msg);
					await Jhon.rejectCall(id.id, id.from)
				}
			}
		}
	});
	
	Jhon.ev.on('messages.upsert', async (message) => {
		await MessagesUpsert(Jhon, message, store, groupCache);
	});
	
	Jhon.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(Jhon, update, store, groupCache);
	});
	
	Jhon.ev.on('groups.update', (update) => {
	...
	Jhon.ev.on('presence.update', ({ id, presences: update }) => {
		store.presences[id] = store.presences?.[id] || {};
		Object.assign(store.presences[id], update);
	});
	
	setInterval(async () => {
		if (Jhon?.user?.id) await Jhon.sendPresenceUpdate('available', Jhon.decodeJid(Jhon.user.id)).catch(e => {})
	}, 10 * 60 * 1000);
